...
export function guest(doc, options) {
	...
	return event => {
		...
		if (nativeEvent && allowGuestEvent(nativeEvent, elem, options)) {
			if (elem.nodeName === 'SELECT' || elem.nodeName === 'OPTION') {
				// console.log(elem, event.data, nativeEvent);
			}
...
/**
 * Edge case: click on <label> element may cause imlicit click on matched
