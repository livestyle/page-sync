...
export default function(element, pseudo, rules=createIndex(element.ownerDocument)) {
    ...
    rules = rules.filter(rule => match(element, rule.selectorText, pseudo));

    // in matched rules, extract only those selectors that match given element
    // then sort them by specificity
    var matchedSelectors = [];
    rules.forEach(rule => {
        splitSelector(rule.selectorText).forEach(selector => {
            if (match(element, selector, pseudo)) {
                matchedSelectors.push({selector, rule});
            }
        });
    });

    return matchedSelectors
    .sort((a, b) => calculateScore(b.selector) - calculateScore(a.selector));
}
...
function isDebug(elem) {
    return elem.classList && elem.classList.contains('button-buy-ticket');
}
